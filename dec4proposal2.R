...
ggplot(var2, aes(x = HT, y = DIA, color = nro)) + geom_point(size = 0.9) + scale_color_gradient(low = "red", high = "green") + labs(title = "Quercys Rubra Variables", x = "Height", y = "Diameter")
...
ggplot(var4, aes(x = LON, y = LAT, color = sweet_b, nro)) + geom_point(size = 1) + scale_color_gradient(low = "grey", high = "black") + labs(title = "Sweet Birch", x = "Longitude", y = "Latitude")
...
ggplot(nro, aes(LON, LAT)) + geom_polygon(nro = nro, fill = "yellow") + coord_map()

library(ggplot2) library(sf) library(tidyverse)

plot(nro) plot(sweet_b)

ggplot(nro, aes(LON, LAT)) + geom_sf(nro = nro, fill = "lightblue") scale_fill_gradient(low = "red", high = "yellow")
